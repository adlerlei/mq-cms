...
### v4.0 版本亮點
- **依賴管理與部署穩定性**：透過建立全新的隔離虛擬環境，解決了全域環境污染問題。產生了一份乾淨、精確的 `requirements.txt`，大幅提升了在新環境中部署的可靠性與速度。 
- **跨平台開發者體驗優化**：解決了在 macOS 上因「AirPlay 接收器」服務佔用 5000 端口而無法啟動的問題，提升了本地開發的便利性。 
- **端口配置靈活性**：將應用程式的預設監聽端口從 `5000` 修改為 `5003`，以避開常見的端口衝突，並將此設定明確化。 
- **伺服器防火牆配置文檔化**：在啟動指南中新增了針對 Ubuntu `ufw` 防火牆的配置說明，確保新端口在伺服器上可以被正常存取。 
